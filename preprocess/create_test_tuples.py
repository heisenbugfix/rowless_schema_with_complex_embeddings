...
org_data_path = "../data/rowless_data/"
data_path = "../data/rowless_data/"

embeddings_size = 50
max_sent_size = 35
test_neg_rel_size = 10

with open(data_path+'word_to_id_map.pickle','rb') as f:
...
with open(org_data_path+'test_ids.pickle','wb') as f:
    pickle.dump(ids,f,protocol=2)
try:
	with open(org_data_path+'test_emb.pickle','wb') as f:
	    pickle.dump(emb,f,protocol=2)
except:
	with open(org_data_path+'test_emb.json','w') as f:
		print('JSONing emb')
		json.dump(emb.tolist(),f)
with open(org_data_path+'test_se_lens.pickle','wb') as f:
...
with open('../data/allrelations.txt') as f:
...
print(len(ent_pair_idx_sent))
print(len(ent_pair_idx_rel))

tuples = create_test_tuples(ent_pair_idx_sent, ent_pair_idx_rel, emb, seq_lens, relations[:,2], test_neg_rel_size)
try:
	with open(data_path+'preprocessed_test.pickle','wb') as f:
		pickle.dump(tuples,f,protocol=2)
except:
	with open(data_path+'preprocessed_test.json','w') as f:
